"""
Anthropic Careers -> LinkedIn Post Pipeline (Greenhouse job board API).
...
log = logging.getLogger("anthropic-jobs")

BOARD_URL = "https://boards-api.greenhouse.io/v1/boards/anthropic/jobs?content=true"
FALLBACK_URL = "https://www.anthropic.com/careers"
...
def fetch_recent_jobs(cutoff):
    ...
    for j in raw:
        ...
        jobs.append(job)
    log.info("Anthropic: %d fresh jobs", len(jobs))
    return jobs
...
def build_post(jobs, date_str, part=None, total_parts=None):
    header = f"✳️ Anthropic is Hiring! | {date_str}"
    ...
    lines += ["♻️ Repost to help someone in your network!",
              "\U0001F514 Follow for daily Anthropic job updates.", "",
              "#AnthropicCareers #AI #Hiring #TechJobs #SoftwareEngineering #NowHiring"]
    return "\n".join(lines)
